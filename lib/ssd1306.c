...
// Função que desenha o cabeçalho do programa no display
void frame(ssd1306_t *ssd, uint16_t vagas)
{
  char ocu[5]; // Vagas oculpadas
  char vag[5]; // Vagas livres
  sprintf(ocu, "%u", vagas);
  sprintf(vag, "%u", 9 - vagas);
  ssd1306_fill(ssd, 0);
  ssd1306_rect(ssd, 0, 0, 128, 64, true, false);
  ssd1306_line(ssd, 1, 12, 126, 12, true);
  ssd1306_line(ssd, 1, 47, 126, 47, true);
  ssd1306_draw_string(ssd, "Biblioteca", 24, 2);
  ssd1306_draw_string(ssd, "Vagas:", 5, 16);
  ssd1306_draw_string(ssd, "Oculpadas:", 5, 26);
  ssd1306_draw_string(ssd, "Livres:", 5, 36);
  ssd1306_draw_string(ssd, "9", 106, 16);
  ssd1306_draw_string(ssd, ocu, 106, 26);
  ssd1306_draw_string(ssd, vag, 106, 36);
  if (vagas < 9)
  {
    ssd1306_draw_string(ssd, "Livre", 43, 52);
  }
  else
  {
    ssd1306_draw_string(ssd, "Oculpado", 33, 52);
  }
